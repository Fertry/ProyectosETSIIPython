...
def calcula_dimensiones(imagen):
    ...
    fila.
    '''
    tupla = [(int(filas),int(columnas)) for filas,columnas,_ in imagen]
...
def calcula_intensidades_medias(imagen):
    ...
    '''
    color_rojo = [rojo for rojo,_,_ in imagen]
    color_verde = [verde for _,verde,_ in imagen]
    color_azul = [azul for _,_,azul in imagen]

    media_rojo = sum(color_rojo) / len(color_rojo)
    media_verde = sum(color_verde) / len(color_verde)
    media_azul = sum(color_azul) / len(color_azul)

    tupla = [(media_rojo,media_verde,media_azul)]
...
def reflejo_vertical(imagen):
    ...
    '''
    res = [[(p3,p2,p1)] for p1,p2,p3 in imagen]
