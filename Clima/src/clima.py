...
def lee_registros(fichero):
    ...
    registros = [Registro(datetime.strptime(fecha,"%Y-%m-%d").date(), float(lluvia), float(temperatura_maxima), float(temperatura_minima)) for fecha,lluvia,temperatura_maxima,temperatura_minima in datos]
...
def filtro_por_periodo(registros, inicio, fin):
    ...
    registros = [(fecha,lluvia,temperatura_maxima,temperatura_minima) for fecha,lluvia,temperatura_maxima,temperatura_minima in registros if fecha > fecha_inicio and fecha < fecha_fin]
...
def medias_por_meses(registros):
    ...
    for Registro.fecha.month in registros:

        for lluvia in registros:

            lista_precipitaciones.append(lluvia)

        for temperatura_maxima in registros:

            lista_temperaturas_maximas.append(temperatura_maxima)

        for temperatura_minima in registros:

            lista_temperaturas_minimas.append(temperatura_minima)

    media_precipitaciones = np.average(lista_precipitaciones)
    media_temperaturas_maximas = np.average(lista_temperaturas_maximas)
    media_temperaturas_minimas = np.average(lista_temperaturas_minimas)

    return media_precipitaciones, media_temperaturas_maximas, media_temperaturas_minimas
    '''
    meses = range(1,13)

    media_lluvia = [np.average([Registro.lluvia for Registro in registros if mes == Registro.fecha.month]) for mes in meses]
    media_temperatura_maxima = [np.average([Registro.temperatura_maxima for Registro in registros if mes == Registro.fecha.month]) for mes in meses]
    media_temperatura_minima = [np.average([Registro.temperatura_minima for Registro in registros if mes == Registro.fecha.month]) for mes in meses]

    return media_lluvia, media_temperatura_maxima, media_temperatura_minima
...
def variaciones_mensuales(historicos, valores_periodo):
    ...
    '''
    lista_variaciones_lluvia = []
    lista_variaciones_temperatura_maxima = []
    lista_variaciones_temperatura_minima = []

    lista_precipitaciones_historicas = []
    lista_precipitaciones_periodicas = []

    lista_temperaturas_maximas_historicas = []
    lista_temperaturas_maximas_periodicas = []

    lista_temperaturas_minimas_historicas = []
    lista_temperaturas_minimas_periodicas = []

    for lluvia in historicos:

        lista_precipitaciones_historicas.append(lluvia)

    for lluvia in valores_periodo:

        lista_precipitaciones_periodicas.append(lluvia)

    for temperatura_maxima in historicos:

        lista_temperaturas_maximas_historicas.append(temperatura_maxima)

    for temperatura_maxima in valores_periodo:

        lista_temperaturas_maximas_periodicas.append(temperatura_maxima)

    for temperatura_minima in historicos:

        lista_temperaturas_minimas_historicas.append(temperatura_minima)

    for temperatura_minima in valores_periodo:

        lista_temperaturas_minimas_periodicas.append(temperatura_minima)

    media_precipitaciones = []
    media_temperaturas_maximas = []
    media_temperaturas_minimas = []

    for valor_periodico_precipitacion in lista_precipitaciones_periodicas:

        for valor_historico_precipitacion in lista_precipitaciones_historicas:

            media_precipitaciones.append(valor_periodico_precipitacion - valor_historico_precipitacion)

    for valor_periodico_temperatura_maxima in lista_temperaturas_maximas_periodicas:

        for valor_historico_temperatura_maxima in lista_temperaturas_maximas_historicas:

            media_temperaturas_maximas.append(valor_periodico_temperatura_maxima - valor_historico_temperatura_maxima)

    for valor_periodico_temperatura_minima in lista_temperaturas_minimas_periodicas:

        for valor_historico_temperatura_minima in lista_temperaturas_minimas_historicas:

            media_temperaturas_minimas.append(valor_periodico_temperatura_minima - valor_historico_temperatura_minima)
...
def variaciones_anuales(historicos, valores_periodo):
    ...
    for Registro.fecha.year in historicos:

        for Registro.fecha.year in valores_periodo:
