...
def construye_diccionario_codificacion(k):
    ...
    for i in range(len(lista_alfabeto)):
        diccionario[lista_alfabeto[i]] = lista_alfabeto[(i+k) % len(lista_alfabeto)]
...
def cifra_mensaje(texto, k):
    ...
    '''
    diccionario = construye_diccionario_codificacion(k)
...
def descifra_mensaje(texto, k):
    ...
    '''
    resultado = cifra_mensaje(texto,-k)
...
def es_palabra_valida(palabra, vocabulario):
    ...
    cierra paréntesis o admiración, etc. Los símbolos a eliminar son:
        " !@#$%^&*()-_+={}[]|\:;'<>?,./\"¡¿"
    '''
    return palabra.strip(" !@#$%^&*()-_+={}[]|\:;'<>?,./\"¡¿").lower() in vocabulario
...
def cuenta_palabras_validas(texto, vocabulario):
    ...
    for palabra in texto.split(" "):
        if es_palabra_valida(palabra,vocabulario) == True:
            resultado += 1
...
def rompe_codigo_cifrado(texto, vocabulario):
    ...
    for k in range(0,27):
        posibles_k.append(k)
        mensaje = descifra_mensaje(texto,k)
        aciertos.append(cuenta_palabras_validas(mensaje,vocabulario))

    return posibles_k[aciertos.index(max(aciertos))],descifra_mensaje(texto,posibles_k[aciertos.index(max(aciertos))])
