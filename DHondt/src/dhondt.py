...
def lee_escrutinio(fichero_votos, fichero_escaños):
    ...
    '''
    f = open(fichero_votos, encoding="utf-8")

    datos_votos = csv.reader(f)

    next(datos_votos)

    tuplitas = [(provincia,partido,int(voto)) for provincia,partido,voto in datos_votos]

    f.close()

    g = open(fichero_escaños, encoding="utf-8")

    datos_escaños = csv.reader(g)

    next(datos_escaños)

    diccionario = {provincia:int(escaño) for provincia,escaño in datos_escaños}

    g.close()

    return tuplitas, diccionario
...
def calcula_provincias(votos):
    ...
    '''
    conjunto = {provincia for provincia,_,_ in votos}
...
def calcula_partidos(votos):
    ...
    '''
    conjunto = {partido for _,partido,_ in votos}

    return conjunto

# EJERCICIO 4:
def calcula_diccionario_provincia(votos, provinciaIntroducida):
    ''' Calcula un diccionario con los votos de cada partido en una provincia
    ...
    '''
    diccionario = {partido:int(voto) for provincia,partido,voto in votos if provincia == provinciaIntroducida}
...
def calcula_diccionario_provincias(votos):
    ...
    '''
    diccionario = {provincia:calcula_diccionario_provincia(votos,provincia) for provincia,_,_ in votos}
...
def genera_mapa_calor(tabla_valores, limite_columnas=None, fmt='d'):
    ...
    Donde 'filas' es una lista de vectores, uno por cada provincia:. 
        ...
        - Los partidos se ordenarán siempre de la misma forma para los vectores de todas
          las provincias.
    '''
    provincias = tabla_valores.keys()

    partidos = tabla_valores.values().keys()

    filas = tabla_valores.values().values()

    nombres_partidos = [partido[:10] for partido in partidos]
    g = sns.heatmap(filas, annot=True, fmt=fmt, cmap="PuBu", xticklabels=nombres_partidos, yticklabels=provincias)
    g.set_yticklabels(g.get_yticklabels(), rotation =0)
    g.set_xticklabels(g.get_xticklabels(), rotation =65)
    plt.show()
...
def calcula_tabla_porcentajes(tabla_votos):
    ...
    '''
    lista_provincias = [tabla_votos.keys()]
    lista_partidos = [tabla_votos.values().keys()]
    lista_votos = [tabla_votos.values().values()]

    contadorDeVotosPorProvincia = 0
    contadorDeVotosPorPartido = 0
    medias_por_partido = []
    ...
    for partido in lista_partidos:
        ...
        for provincia in lista_provincias:
            ...
            for voto in lista_votos:
                ...
                contadorDeVotosPorProvincia += 1

        for partido in lista_partidos:

            for voto in lista_votos:

                contadorDeVotosPorPartido += voto

        media = contadorDeVotosPorPartido / contadorDeVotosPorProvincia

        medias_por_partido.add(media)

    diccionario_interno = {lista_partidos : medias_por_partido}
    resultado = {lista_partidos:diccionario_interno}
