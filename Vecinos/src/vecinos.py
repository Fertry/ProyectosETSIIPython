...
def lee_dataset_iris(fichero):
    ...
    lista_tuplas_numericas = [(float(sepal_length), float(sepal_width), float(petal_length), float(petal_width)) for sepal_length, sepal_width, petal_length, petal_width,_ in datos]

    lista_especies = [species for _,_,_,_,species in datos]

    dataset = Dataset([[lista_tuplas_numericas]], lista_especies)
...
def calcula_distancia(v1, v2, distancia='euclidean'):
    ...
    if distancia == "euclidean":
        ...
        resultado = distance.euclidean(v1, v2)
    
    elif distancia == "cityblock" or distance == "manhattan":

        resultado = distance.cityblock(v1, v2)

    elif distancia == "canberra":

        resultado = distance.canberra(v1, v2)

    elif distancia == "chebyshev":

        resultado = distance.chebyshev(v1, v2)
...
def vecinos_mas_cercanos(atributos, train, k=3, distancia='euclidean'):
    ...
    for i, t in enumerate(train.atributos):

        resultado.append((calcula_distancia([t[0],t[1]], [t[2],t[3]], distancia), t, train.clases[i]))

    return sorted(resultado, reverse=True)[:k]
